...
function RecipePage({recipes, user, setUser, reviews}) {
  ...
  const userReviews = reviews.filter(r => r.recipe_id === parseInt(id)).reverse().map( r => <p>{`${r.user} said: "${r.description}"`}</p>)
  ...
  return (
    <div className='recipe-page'>
      ...
      <h1 align='center'>{recipe.name}</h1>
        ...
        {leaveReview ? reviewForm : <button onClick={()=>handleClick()}>Leave a Review</button> }
    </div>
  )
}
